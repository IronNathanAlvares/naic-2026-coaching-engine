  ...
  sections: [
    {
      ...
      children: [
        ...
        beat(
          [{ label: "ON SCREEN", text: "Three cards with arrows between them. Underneath: LMS shows completion, role-play scores practice, Coaching Engine connects practice to behaviour." },
           { label: "YOU", text: "Walk the slide left to right with it, then STEP AWAY from it for the argument. The screen explains the what; you are there for the so what." }],
          [act("go to the screen, hand flat under card 01"),
           say("Two signals. One: what somebody can do in practice, scored against their own hotel's standards."),
           act("move right with the arrow, hand under card 02"),
           say("Two: what their manager actually saw on the floor. Twenty seconds, after service."),
           ...
           act("small shrug"),
           say("So can I.", true),
           act("stop. two hands apart, wide"),
           say("The distance between those two is the coaching signal. Nobody is measuring it."),
           act("hands down, straight at them, slower"),
           say("And what that distance tells you to do is almost never: send him on another course.", true)]
        ),
        note("About fifty seconds and 127 words, which is the budget exactly. The two laughs are throwaways, not setups: say them at pace and keep moving. If the room does not react, nothing has gone wrong, because both lines are also just true."),
        beat(
          [{ label: "HANDOVER", text: "into the demo. Start the video as you say the last word, not after it." }],
          [act("one step back toward the laptop, eyes still on them"),
           say("Let me show you, on a real shift.", true)],
          { shade: "F7F9F9" }
        ),
        note("This is the only joke in the pitch now. The one written at 1:30 in the demo comes out unless this one landed and the room is warm: two jokes in seven minutes reads as a team that is not sure of its numbers."),
